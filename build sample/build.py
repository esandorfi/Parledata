...
logger = zen.initlog(int(args.verbose), "PARLADATA")
...
myIdx = zen.PlwScan(myZen, static_idx_path)
if args.idx == '1':
    thisidx = "data-index.json"
    idxfilename = myIdx.scan("data", ".md", thisidx)
    if( idxfilename == ''):
        logger.critical("Error in idx generation")
        sys.exit(1)
    myZen.addidx(thisidx, idxfilename)
    myZen.route("ui\\idx.md", "idx", "idx-test")
    #del myIdx
    print("SYS EXIT")
    sys.exit(0)
...
myZen.route("plw-vitrine\\landingv2.md", "landingv3", "index")
